package com.sea.sbms.account.dao.impl;
...
import com.sea.sbms.account.bean.Account;
import com.sea.sbms.account.bean.AccountDetail;
import com.sea.sbms.account.dao.AccountDao;
...
public class AccountDaoImpl implements AccountDao {
		...
		try {
			...
			while (rs.next()) {
				accountDetail = new AccountDetail(rs.getInt("accountId"),
						rs.getString("goodsName"), rs.getInt("goodsNum"),
						rs.getInt("businessNum"), rs.getFloat("totalPrice"),
						rs.getInt("isPayed"), rs.getString("providerName"),
						rs.getString("goodsIntro"), rs.getDate("accountDate"));
		...
		if (wheres != null && wheres.size() > 0) {
			...
			for (int i = 0; i < wheres.size(); i++) {
				if (wheres.get(i).equals("goodsName")) {
					sql += "g." + wheres.get(i) + " = " + values.get(i)
							+ " and ";
				} else {
					sql += "a." + wheres.get(i) + " = " + values.get(i)
							+ " and ";
				}
			}
			sql += "1 = 1";
	...
	public PageBean getAccount(PageBean pageBean, List<String> wheres,
		...
		if (wheres != null && wheres.size() > 0) {
			for (int i = 0; i < wheres.size(); i++) {
				if (wheres.get(i).equals("goodsName")) {
					conditions += "g." + wheres.get(i) + " = " + values.get(i)
							+ " and ";
				} else {
					conditions += "a." + wheres.get(i) + " = " + values.get(i)
							+ " and ";
				}
			}
		}
